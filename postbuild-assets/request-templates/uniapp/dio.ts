/**
 * uni-app 版 dio 实例：基于 uni.request（跨端 H5/小程序/App）。
 * 与 axios 版保持相同的对外形态：`dio.request(config)` 与 `dio.interceptors.request|response`。
 * 若 TS 报 `Cannot find name 'uni'`，在项目执行：`npm i -D @dcloudio/types`，并在 tsconfig `types` 中加入 `@dcloudio/types`。
 */

// uni-app 环境变量：process.env.VUE_APP_XXX / import.meta.env.VITE_XXX，根据你的构建配置选一种
