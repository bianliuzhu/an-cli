...
import { getEnumSegment, getServerSegment } from '../shared/naming';
...
export class ComponentWriter {
	...
	async writeEnums(enumsMap: Map<string, RenderEntry>): Promise<void> {
		const exportFileContent: string[] = [];
		const enumSegment = getEnumSegment(this.config);
		const enumSegmentSuffix = enumSegment ? `${enumSegment}/` : '';
		const saveEnumFolderPath = `${this.config.saveEnumFolderPath}/${enumSegmentSuffix}`;
		...
		await runWithConcurrency(sortedEntries, FILE_WRITE_CONCURRENCY, async ({ fileName, content }) => {
			exportFileContent.push(`export * from './${fileName}';`);
			const _path = `${saveEnumFolderPath}${fileName}.ts`;
			// 未启用 enum 隔离（共享枚举目录）时，appendMode 下若已存在同名 enum 但内容不同，发出警告以避免静默覆盖
			if (this.appendMode && !enumSegment) {
				try {
					const prev = await fs.promises.readFile(_path, 'utf8');
					if (prev !== content) {
						log.warning(`Enum 文件冲突：${_path} 已存在且内容不同，将被当前服务覆盖。请检查不同 swagger 服务是否定义了同名但语义不一致的枚举，或开启 enumIsolation: 'segment'。`);
					}
				} catch {
					// 文件不存在，正常写入
				}
		...
		// exportFileContent 也需要排序以确保 index.ts 中的导出顺序一致
		exportFileContent.sort();
		// 隔离写入子目录时，appendMode 仍以 dedup 形式合并；隔离独立目录天然不会与其他服务冲突。
		await writeIndexFileWithDedup(`${saveEnumFolderPath}index.ts`, exportFileContent, { appendMode: this.appendMode });
