...
- 首次生成的文件中包含 axios 的封装配置文件： `config.ts`, `error-message.ts`, `fetch.ts`, `api-type.d.ts`
  - 配置文件名称不可修改，但配置文件里的内容可修改
  - 每一次执行生成命令时，都会检查配置文件是否有存在，不存在的话会自动创建
  - 如果配置文件存在的话，将不再重复生成
...
│   │    └── config.ts       				# 请求、响应拦截、请求配置 用户可修改，不能删除和重命名 不受控
│   │    └── error-message.ts   		# 系统级错误提示 用户可修改，不能删除和重命名 不受控
│   │    ├── fetch.ts        				# axios 请求封装，用户可修改，不能删除和重命名 可换成 fetch 不受控
