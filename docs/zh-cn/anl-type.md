...
package.json（含 prettier 字段）
```

##### 使用示例

```bash
# 自动检测项目中的 prettier 配置并格式化
$ anl type --format
$ anl type -f
...
| swaggerConfig[].timeout                              | number                                                                          | 否   | 请求 Swagger JSON 数据的超时时间（毫秒），默认 `15000`（15 秒）。若未设置，使用全局 `timeout` 配置                                                                                                                                                                                                                                                                                          |
...
| timeout                                              | number                                                                          | 否   | 全局请求 Swagger JSON 数据的超时时间（毫秒），默认 `15000`（15 秒）。各服务器可单独配置覆盖                                                                                                                                                                                                                                                                                                 |
...
│   │    └── op.ts           				# 多服务器时，其他服务器的 API 列表文件 由 swaggerConfig[*].apiListFileName 指定
│   │    └── config/         				# axios 封装配置目录（存在则不重复生成）
│   │         ├── api-type.d.ts     		# 请求类型定义文件，用户可修改，不能删除和重命名 不受控
│   │         ├── dio.ts           		# axios 实例与拦截器配置，用户可修改，不能删除和重命名 不受控
...
在 `apps/api/config/api-type.d.ts` 中定义统一的响应类型：
...
3. 生成的文件会覆盖已存在的同名文件（但 `saveApiListFolderPath/config/` 目录若已存在则不会重复生成其中的 `dio.ts`、`error-message.ts`、`fetch.ts`、`api-type.d.ts`）
4. 建议将生成的文件加入版本控制
...
8. **生成的文件被覆盖了怎么办？**
   - `saveApiListFolderPath/config/` 目录只会在首次不存在时生成（包含 `dio.ts`、`error-message.ts`、`fetch.ts`、`api-type.d.ts`）
