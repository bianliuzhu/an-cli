...
#### Auto-format generated files after generation (-f / --format)
...
```bash
# Auto-detect project prettier config and format
$ anl type --format
$ anl type -f

# Specify a prettier config file path
$ anl type --format .prettierrc.mjs
$ anl type -f prettier.config.ts
$ anl type -f "configs/.prettierrc.json"
...
- **When enum isolation is enabled** (`enumIsolation: 'segment'`, default): the selected services' `enums/<segment>` directory is also cleaned. **When disabled** (`enumIsolation: 'none'`): the shared top-level enum directory is never wiped (so other services' enums are preserved)
...
> - `saveEnumFolderPath/{segment}/*.ts` (enabled by default; disable per-service via `enumIsolation: 'none'`)
>
> e.g. `apiListFileName: 'bff.ts'` → segment `bff` → `apps/types/models/bff/`, `apps/types/connectors/bff/`, `apps/enums/bff/`.
>
> Notes:
>
...
> 3. Use a relative path for `importEnumPath` (e.g. `'../../enums'`). When isolation is enabled, the tool automatically compensates the extra directory depth (prepends one more `../`) and appends `/<segment>` to the path.
> 4. **Enum data isolation** (`enumIsolation`, default `'segment'`): in multi-service mode, enums are written to `${saveEnumFolderPath}/<segment>/` and a top-level namespace barrel is generated:
>    ```ts
>    // apps/enums/index.ts
>    export * as Op from './op';
>    export * as Bff from './bff';
>    ```
>    Two usage patterns on the consumer side:
>    ```ts
>    import { Op } from '@/enums';
>    const s: Op.Status = Op.Status.Enabled;
>    // Or use the flat barrel (no name clash within a single service):
>    import { Status } from '@/enums/op';
>    ```
>    To preserve the legacy behavior of "all services share the top-level enum folder", set `enumIsolation: 'none'` on a service — its enums will still be written to the top level; on duplicate names, the last writer wins. Use with care.
> 5. When upgrading from the legacy (shared-enum) layout, the tool scans `${saveEnumFolderPath}` for stale top-level `*.ts` files and emits a warning recommending a full regen or manual cleanup. The tool itself never deletes them, to avoid clobbering user-managed artifacts.
...
| swaggerConfig                                        | object \| Array<object>                                                         | No       | Swagger server configuration. Single server can be an object, multiple servers use an array. Each server can configure `url`, `publicPrefix`, `modulePrefix`, `apiListFileName`, `headers`, `dataLevel`, `parameterSeparator`, `includeInterface`, `excludeInterface`, `includeTags`, `excludeTags`, `timeout`, `responseModelTransform`<br />See single and multiple Swagger server configuration examples above     |
...
| swaggerConfig[].timeout                              | number                                                                          | No       | Timeout for fetching Swagger JSON data (in milliseconds), defaults to `60000` (60 seconds). If not set, uses global `timeout` configuration                                                                                                                                                                                                                                                                           |
| swaggerConfig[].namespaceIsolation                   | `'segment'` \| `'none'`                                                         | No       | Namespace prefix strategy. Default `'segment'`: derive a PascalCase prefix from `apiListFileName` (e.g. `op.ts` → `Op_`) to avoid `declare namespace` global merging across services that share the same path. Set to `'none'` to disable. Falls back to global `namespaceIsolation`                                                                                                                                  |
| swaggerConfig[].enumIsolation                        | `'segment'` \| `'none'`                                                         | No       | Enum data isolation strategy. Default `'segment'`: in multi-service mode, write enums to `${saveEnumFolderPath}/<segment>/` and emit a top-level namespace barrel, so duplicate enum names across services never collide. Set to `'none'` to share the top-level enum folder (last writer wins on duplicates). Falls back to global `enumIsolation`. **Has no effect in single-service mode (always flat top-level)** |
...
| timeout                                              | number                                                                          | No       | Global timeout for fetching Swagger JSON data (in milliseconds), defaults to `60000` (60 seconds). Each server can override individually                                                                                                                                                                                                                                                                              |
| namespaceIsolation                                   | `'segment'` \| `'none'`                                                         | No       | Global namespace prefix strategy, default `'segment'`. Each server can override via `swaggerConfig[].namespaceIsolation`                                                                                                                                                                                                                                                                                              |
| enumIsolation                                        | `'segment'` \| `'none'`                                                         | No       | Global enum isolation strategy, default `'segment'` (in multi-service mode, enums are written into per-segment subdirectories and a top-level namespace barrel is generated). Each server can override via `swaggerConfig[].enumIsolation`. See "Multi-server output isolation" above                                                                                                                                 |
...
│        ├── index.ts        				# Top-level namespace barrel, auto-generated when multi-service + enumIsolation is enabled
│        └── <segment>/      				# Per-service enum subdirectory in multi-service mode (derived from apiListFileName)
```
...
- `includeTags`: Interfaces to include by tag
- `excludeTags`: Interfaces to exclude by tag
...
#### Interface Filtering by Tag

In addition to filtering by path/method, the tool supports filtering by OpenAPI **tag**, so you can include or exclude interfaces by module (e.g. "User module", "File upload APIs"):

- **`includeTags`**: Only generate interfaces that belong to the specified tags (applied after `includeInterface`/`excludeInterface`).
- **`excludeTags`**: Exclude interfaces that belong to the specified tags.
- `includeTags` and `excludeTags` are mutually exclusive; do not configure both.
- Priority: interface-level `includeInterface`/`excludeInterface` is applied first, then tag filtering.

**Example (under one entry of `swaggerConfig`):**
	...
	"swaggerConfig": [
		{
			...
			"apiListFileName": "index.ts",
			"includeTags": ["File upload APIs"]
		},
		{
			"url": "https://api2.example.com/openapi.json",
			"apiListFileName": "op.ts",
			"excludeTags": ["Internal debug APIs"]
		}
	]
}
```
